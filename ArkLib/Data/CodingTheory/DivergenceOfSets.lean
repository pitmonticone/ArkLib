...
theorem Pr_uniform_eq_one_imp_forall {α : Type} [Fintype α] [Nonempty α] (P : α → Prop) :
  ...
  have hFalse : False ∈ q.support := by
    ...
    refine ⟨a, ?_, hPfalse⟩
    simpa using (PMF.mem_support_uniformOfFintype (α := α) a)
  have : False ∈ ({True} : Set Prop) := by
    simpa [hsupport] using hFalse
...
theorem Pr_uniform_equiv {α β : Type} [Fintype α] [Nonempty α] [Fintype β] [Nonempty β]
  ...
  have hmap : (PMF.uniformOfFintype α).map e = PMF.uniformOfFintype β := by
    ...
    simp [PMF.map_apply, PMF.uniformOfFintype_apply, Fintype.card_congr e]
    -- Make the sum explicit (avoid `∑` notation).
    change
      Finset.univ.sum (fun a : α =>
        if b = e a then (Fintype.card β : ENNReal)⁻¹ else 0) = (Fintype.card β : ENNReal)⁻¹
    ...
    have hdelta :
        Finset.univ.sum (fun b' : β =>
            if b = b' then (Fintype.card β : ENNReal)⁻¹ else 0) = (Fintype.card β : ENNReal)⁻¹ := by
      simpa using
        (Fintype.sum_ite_eq (i := b)
          (f := fun _ : β => (Fintype.card β : ENNReal)⁻¹))
  ...
  have hcomp : (PMF.uniformOfFintype α).map (P ∘ e) = ((PMF.uniformOfFintype α).map e).map P := by
  ...
  simpa using congrArg (fun q : PMF Prop => q True) (by
    calc
      ...
      _ = (PMF.uniformOfFintype β).map P := by simpa [hmap]
    )
...
theorem proximity_gap_affineSubspace {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  ...
  Xor'
    ...
    (Pr_{let u ← $ᵖ U}[Code.relDistFromCode u (RScodeSet domain deg) ≤ δ] ≤ errorBound δ deg domain) := by
  classical
  ...
  haveI hS_nonempty : Nonempty S := by
    ...
    have hx_mem_image : (x : ι → F) ∈ (Finset.univ.image u : Finset (ι → F)) := by
      ...
      simp [u]
    have hx_mem_affineSpan : (x : ι → F) ∈ affineSpan F (Finset.univ.image u : Set (ι → F)) :=
      (subset_affineSpan (k := F) (s := (Finset.univ.image u : Set (ι → F)))) hx_mem_image
  ...
  have h_affineSpan : affineSpan F (Finset.univ.image u : Set (ι → F)) = U := by
    calc
      affineSpan F (Finset.univ.image u : Set (ι → F)) = affineSpan F (U : Set (ι → F)) := by
        simpa [hu_image]
      _ = U := by
        simpa using (AffineSubspace.affineSpan_coe (k := F) (s := U))
...
theorem real_sqrt_div_10_pow_two {r : ℝ≥0} : (Real.sqrt (r : ℝ) / 10) ^ 2 = (r : ℝ) / 100 := by
  have hr : (0 : ℝ) ≤ (r : ℝ) := r.2
  calc
    ...
    _ = (r : ℝ) / 100 := by
      ...
      simpa [Real.sq_sqrt hr, h10]

theorem real_sqrt_rate_eq_coe_nnreal_sqrt {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  {F : Type} [Fintype F] [Field F] [DecidableEq F]
  {deg : ℕ} {domain : ι ↪ F} :
  ...
  simpa using
    (Real.coe_sqrt
        (x := (LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0))).symm

theorem reedSolomon_dim_le_deg {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  {F : Type} [Fintype F] [Field F] [DecidableEq F]
  {deg : ℕ} {domain : ι ↪ F} :
...
theorem reedSolomon_rate_le_one {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  {F : Type} [Fintype F] [Field F] [DecidableEq F]
  {deg : ℕ} {domain : ι ↪ F} :
  ...
  refine div_le_one_of_le₀ ?_ (by
    simpa using (zero_le (LinearCode.length (ReedSolomon.code domain deg) : ℚ≥0)))
  have hdim_le_len_nat :
    ...
    simpa [Module.finrank_pi (R := F)] using h_le
  exact_mod_cast hdim_le_len_nat

theorem reedSolomon_rate_mul_card_eq_dim {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  {F : Type} [Fintype F] [Field F] [DecidableEq F]
  {deg : ℕ} {domain : ι ↪ F} :
  ...
  simp [LinearCode.rate, LinearCode.length, LinearCode.dim] at *
  ...
  rw [← mul_div_assoc]
  simpa [mul_assoc] using (mul_div_cancel_left₀ (b := (Module.finrank F ↥(ReedSolomon.code domain deg) : ℝ≥0)) (a := (Fintype.card ι : ℝ≥0)) hn)

theorem reedSolomon_rate_mul_card_le_deg {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  {F : Type} [Fintype F] [Field F] [DecidableEq F]
  {deg : ℕ} {domain : ι ↪ F} :
  (Fintype.card ι : ℝ≥0) * (LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0)
    ≤ (deg : ℝ≥0) := by
...
theorem reedSolomon_rate_pos {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  {F : Type} [Fintype F] [Field F] [DecidableEq F]
  {deg : ℕ} {domain : ι ↪ F}
  (hdeg : 0 < deg) :
  ...
  have hmem : ReedSolomonCode.constantCode (1 : F) ι ∈ ReedSolomon.code domain deg := by
    simpa using (ReedSolomonCode.constantCode_mem_code (ι := ι) (F := F)
      (x := (1 : F)) (α := domain) (n := deg))
  ...
  have hlen_pos : 0 < LinearCode.length (ReedSolomon.code domain deg) := by
    simpa [LinearCode.length] using (Fintype.card_pos (α := ι))
...
theorem errorBound_ge_const {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  {F : Type} [Fintype F] [Field F] [DecidableEq F]
  {deg : ℕ} {domain : ι ↪ F}
  (hdeg : 0 < deg)
  {δ : ℝ≥0}
  ...
  ·
    simpa [ProximityGap.errorBound, ReedSolomonCode.sqrtRate, ←hr, hUD]
  ·
    ...
    simp [ProximityGap.errorBound, ReedSolomonCode.sqrtRate, ←hr, hUD, hmem2]
    ...
    have hm_le : m ≤ Real.sqrt (r : ℝ) / 20 := by
      simpa [hm] using (min_le_right _ _)
    ...
    have hr_pos : (0 : ℝ) < (r : ℝ) := by
      ...
      have hrate_pos : (0 : ℝ≥0) < (LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0) := by
...
theorem errorBound_johnson_mono {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  {F : Type} [Fintype F] [Field F] [DecidableEq F]
  {deg : ℕ} {domain : ι ↪ F}
  (hdeg : 0 < deg)
  {δ₁ δ₂ : ℝ≥0}
  ...
  simp [ProximityGap.errorBound, Set.mem_Icc, Set.mem_Ioo,
    t, (not_le_of_gt ht1), (not_le_of_gt ht2), ht1, ht2, hδ₁', hδ₂']

  -- Turn the subtype inequality into a plain real inequality.
  change (↑deg ^ 2 /
        ((2 * min (↑(1 - (LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0).sqrt - δ₁) : ℝ)
  ...
  set A₁ : ℝ := (↑(1 - (LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0).sqrt - δ₁) : ℝ)
  set A₂ : ℝ := (↑(1 - (LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0).sqrt - δ₂) : ℝ)
  set B : ℝ := (Real.sqrt ((LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0) : ℝ) / 20)
  ...
  have hdenpos : 0 < (2 * min A₂ B) ^ 7 * (Fintype.card F : ℝ) := by
    ...
    have hBpos : 0 < B := by
      -- `B = sqrt(rate)/20`
      have : 0 < Real.sqrt ((LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0) : ℝ) := by
        -- rate is positive
        have hratepos' : 0 < ((LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0) : ℝ) := by
          exact_mod_cast hratepos_nn
        exact Real.sqrt_pos.2 hratepos'
      have : 0 < Real.sqrt ((LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0) : ℝ) / 20 := by
        exact div_pos this (by norm_num)
      simpa [B] using this
    have hA2pos : 0 < A₂ := by
      -- from `δ₂ < 1 - sqrtRate`, the NNReal subtraction is positive, then cast
      have hA2pos_nn : 0 < (1 - (LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0).sqrt - δ₂) := by
        simpa [sub_eq_add_neg, tsub_tsub] using (tsub_pos_of_lt hδ₂')
      -- cast to ℝ
      have : (0 : ℝ) < (↑(1 - (LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0).sqrt - δ₂) : ℝ) := by
...
theorem errorBound_mono {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  {F : Type} [Fintype F] [Field F] [DecidableEq F]
  {deg : ℕ} {domain : ι ↪ F}
  (hdeg : 0 < deg)
  {δ₁ δ₂ : ℝ≥0}
  ...
  · have h1 : δ₁ ≤ t := le_trans hδ h2
    ...
    simp [ProximityGap.errorBound, t, ρ, Set.mem_Icc, Set.mem_Ioo, h1, h2]
  · have ht2 : t < δ₂ := lt_of_not_ge h2
    ...
    · have ht1 : t < δ₁ := lt_of_not_ge h1
      ...
      have hmono : errorBound δ₁ deg domain ≤ errorBound δ₂ deg domain := by
        ...
        have ht1' : (1 - (LinearCode.rate (ReedSolomon.code domain deg) : ℝ≥0)) / 2 < δ₁ := by
          simpa [t, ρ] using ht1
        exact
          errorBound_johnson_mono (deg := deg) (domain := domain) hdeg (δ₁ := δ₁) (δ₂ := δ₂) hδ ht1' hδ₂
...
theorem divergence_pred_spec {ι : Type} [Fintype ι] [Nonempty ι]
  ...
  ∃ δ : ℚ≥0, δ < divergence U V ∧ (∀ u ∈ U, δᵣ'(u, V) ≠ divergence U V ↔ δᵣ'(u, V) ≤ δ) := by
  classical
  ...
  have hδlt : δ < δ' := by
    ...
    · have hposR : (0 : ℝ≥0) < (δ' : ℝ≥0) := by
      ...
      have hposQ : (0 : ℚ≥0) < δ' := by
        exact (NNRat.cast_lt (K := ℝ≥0) (p := (0 : ℚ≥0)) (q := δ')).1 (by simpa using hposR)
      simpa [δ, h] using hposQ

  refine ⟨δ, ?_, ?_⟩
  · simpa [δ'] using hδlt
  · intro u hu
    have hu_le : δᵣ'(u, V) ≤ divergence U V := relDistFromCode'_le_divergence (U := U) (V := V) u hu
    ...
    have hlt_iff_leδ : δᵣ'(u, V) < δ' ↔ δᵣ'(u, V) ≤ δ := by
      ...
      · intro hlt
        ...
        have hle_max : δᵣ'(u, V) ≤ Slt.max' hnonempty := Finset.le_max' (s := Slt) (x := δᵣ'(u, V)) hmemSlt
        -- unfold δ
...
theorem concentration_bounds {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
  ...
  have cast_ennreal_eq_iff (p q : ℚ≥0) : ((p : ENNReal) = (q : ENNReal)) ↔ p = q := by
    ...
    simpa [ENNReal.coe_inj] using (NNRat.cast_inj (α := ℝ≥0) (p := p) (q := q))

  have cast_ennreal_le_iff (p q : ℚ≥0) : ((p : ENNReal) ≤ (q : ENNReal)) ↔ p ≤ q := by
    change (((p : ℝ≥0) : ENNReal) ≤ ((q : ℝ≥0) : ENNReal)) ↔ p ≤ q
    simpa [ENNReal.coe_le_coe] using (NNRat.cast_le (K := ℝ≥0) (p := p) (q := q))
